...
class StudentRepository
{
    public function dataTable($request)
    {
        $items = Student::select(
            'profile_students.core_person_id as id', 'profile_students.is_active',
        ...
        if (empty($request->sortBy)) { $items->orderBy('profile_students.core_person_id', 'desc'); }
        ...
        if (!empty($search)) {
            $items->where(function ($q) use ($search) {
                $q->where('core_persons.name', 'like', "%{$search}%")
                    ...
                    ->orWhere(DB::raw('CONCAT(core_persons.name, " ", core_persons.paternal_surname)'), 'like', "%{$search}%");
            });
        }
