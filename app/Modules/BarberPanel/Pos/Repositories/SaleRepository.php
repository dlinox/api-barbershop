...
namespace App\Modules\BarberPanel\Pos\Repositories;
...
{
    private function getBarberInfrastructureId(): int
    {
        ...
        $infrastructure = Infrastructure::where('infrastructurable_type', 'barbershop_branches')
            ->where('infrastructurable_id', $barber->branch_id)
            ->first();

        if (!$infrastructure) {
            throw new ApiException('La sede del barbero no tiene infraestructura configurada.', 404);
        }
    ...
    {
        $infraId = $this->getBarberInfrastructureId();
        ...
        $query = Sale::select(
            ...
            ->where('treasury_cash_registers.infrastructure_id', $infraId);
    ...
    {
        return Sale::with(['items.presentation.product'])->findOrFail($id);
    }

    public function salesOverview(int $cashSessionId): array
    {
        $stats = Sale::selectRaw('status, COUNT(*) as count, COALESCE(SUM(total), 0) as amount')
